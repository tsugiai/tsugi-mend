...
## Straggler-aware incremental collect (opt-in, default OFF)

By default the syncer stores every fragment and does ALL merge arithmetic at finalize, so under a laggard learner the healthy fragments sit idle through the grace window and the full merge cost lands on the critical path after the last arrival. With `incremental_collect=True` on `GraceWindowSyncer` (knob: `MendConfig.incremental_collect`), each accepted fragment is folded into a float64 running accumulation at arrival time, i.e. during the wait, so only the laggard's own term plus the final divide-and-cast remain after the last arrival. The idea is adapted from straggler-aware collective scheduling (arXiv:2505.23523) to this SDK's fragment/merge layer.

- **Bit-exact by construction, not by tolerance.** Floating-point addition is not associative, so arbitrary incremental accumulation would change the merged bits. This mode instead replays the exact op sequence of the frozen `token_weighted_merge` / `uniform_merge` functions in their exact effective order: those functions iterate `fragments.values()`, i.e. dict insertion order, which IS first-arrival order, so folding each fragment in at arrival executes the byte-identical float64 op chain the frozen path would execute at finalize. The "canonical-order prefix that has fully arrived" is therefore always the entire arrived set. The ordering-proof tests in `tests/test_incremental_collect.py` assert `torch.equal` AND raw-bit identity (integer views, which catch `-0.0` vs `+0.0`) against the default path across arrival orders (canonical, reverse, random, laggard-last, laggard-first), learner counts (2/3/4/8), both merge variants, and fp32/bf16 adversarial payloads (denormals, large/small magnitude mix, `-0.0`).
- **Fallback when order-identity cannot be proven.** The round abandons the accumulation and runs the frozen finalize-time merge (reproducing default results, error messages, and error timing exactly) on: a resubmitted fragment (the dict keeps the learner's original position with the new value, which float arithmetic cannot bit-exactly undo), a parameter-count mismatch (the frozen path owns that `ValueError` at finalize), any error raised mid-accumulation, a non-positive token total, or a finalize-time integrity-check failure -- the frozen functions read fragments at finalize time, so the syncer verifies that nothing was rebound and no API-level in-place op touched an accumulated tensor (PyTorch version counters; raw-storage pokes through `.data` are out of scope: fragments are owned by the syncer once submitted). `GraceWindowSyncer.last_merge_used_incremental` reports whether the last round used the accumulation (`True`), fell back (`False`), or had the mode off (`None`).
- **Default OFF preserves the historical path.** With the mode off the merge work happens exactly where it always did, and the frozen merge functions remain the untouched semantic reference for both modes.
- **Honest scope.** This changes WHEN the merge arithmetic happens, not WHAT is computed; the win is critical-path latency under a laggard, measured so far only at CPU scale (see the PR's micro-benchmark note). The knob is consumed by `GraceWindowSyncer`'s constructor; threading `MendConfig.incremental_collect` through the runtime / `ConcurrentOuterStep` wiring is a separate follow-up change. A real throughput claim waits for a maintainer-run GPU measurement.
