...
- **Default `None` is byte-for-byte the historical behavior.** With no roster the runtime calls `start_round(round_id)` exactly as before: quorum, then the full grace window, and an empty `learners_absent`. There is no new code path on the default branch.
...
- **Safe fallback on a misdeclared roster.** A roster naming learners that never arrive is inert by construction in the syncer (early-finalize never fires; the round falls through to grace expiry and the absentees show up in `learners_absent`). A roster too small to satisfy quorum (which would make `start_round` raise) is rejected at config-init time for the config-level roster, and defensively dropped to the `None` path with a warning for a per-round override. In no case does a misdeclared roster hang the round or change the merged result.
