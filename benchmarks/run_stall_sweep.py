...
# The quick smoke spans a no-stall point (0 ms) and a meaningful stall point
# (100 ms) so it shows the honest curve shape in one cheap run: near-zero at no
# stall, a clear recovery once a peer straggles. The 0 ms point keeps the smoke
# honest about the regime where the SDK does not pay for itself.
QUICK_DELAYS_MS = (0, 100)
...
def _base_config(args: argparse.Namespace) -> BenchConfig:
    ...
    cfg.steps = args.steps if args.steps is not None else (40 if quick else cfg.steps)
    cfg.warmup_steps = (
        args.warmup_steps if args.warmup_steps is not None else (8 if quick else cfg.warmup_steps)
    )
    cfg.sync_period_steps = (
        args.sync_period_steps
        if args.sync_period_steps is not None
        else (10 if quick else cfg.sync_period_steps)
    )
    cfg.apply_lag_steps = (
        args.apply_lag_steps
        if args.apply_lag_steps is not None
        else (6 if quick else cfg.apply_lag_steps)
    )
    cfg.seed = args.seed
    # Quick workload is sized so a single inner step costs a few ms on CPU: large
    # enough that the SDK's overlap of the real cross-rack merge is measurable and
    # the uplift numbers are interpretable, rather than a sub-millisecond regime
    # where async overhead dominates and the smoke prints noisy negative uplift.
    cfg.batch = args.batch if args.batch is not None else (128 if quick else cfg.batch)
    cfg.in_dim = args.in_dim if args.in_dim is not None else (256 if quick else cfg.in_dim)
    cfg.hidden = args.hidden if args.hidden is not None else (512 if quick else cfg.hidden)
    cfg.out_dim = args.out_dim if args.out_dim is not None else (64 if quick else cfg.out_dim)
    cfg.sequence_length = (
        args.sequence_length
        if args.sequence_length is not None
        else (64 if quick else cfg.sequence_length)
    )
